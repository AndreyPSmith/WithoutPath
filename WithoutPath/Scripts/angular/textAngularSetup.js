    ...
    return {
        ...
        aOnSelectAction: function (event, $element, editorScope) {
            ...
            reLinkButton.on('click', function (event) {
                ...
                editorScope.title = taTranslations.insertLink.dialogPrompt;
                editorScope.urlLink = $element.attr('href');
                ...
                template += "<legend><i class='fa fa-link'></i> {{title}} </legend>";

                template += "<input tabindex='-1' class='form-control'  ng-model='urlLink'  type='text'>";

                template += "</div><div class='modal-footer'>";
                template += "<button  ng-click='Ok(urlLink)' class='btn btn-primary btn-login-popap'>Ok</button></div>";
                ...
                editorScope.Ok = function (urlLink) {
    ...
    function (taRegisterTool, $window, taTranslations, taSelection, taToolFunctions, $sanitize, taOptions, $log, $uibModal) {
        ...
        taRegisterTool('insertImage', {
            ...
            action: function () {
                ...
                var sel = taSelection.getApiSelection();

                this.title = taTranslations.insertImage.dialogPrompt;
                this.imageLink = 'http://';
                ...
                template += "  </div>";
                template += "<div class='modal-body'>";
                template += "<legend><i class='fa fa-picture-o'></i> {{title}} </legend>";

                template += "<input tabindex='-1' class='form-control'  ng-model='imageLink'  type='text'>";

                template += "</div><div class='modal-footer'>";
                template += "<button  ng-click='Ok(imageLink)' class='btn btn-primary btn-login-popap'>Ok</button></div>";
                ...
                this.Ok = function (imageLink) {

                    if (sel.end.offset == sel.start.offset) {
                        taSelection.setSelection(el, el, 1, 1);
                    }
                    else {
                        if (sel.start.offset == 0 && sel.end.offset > sel.start.offset + 2) {
                            taSelection.setSelection(sel.start.element, sel.end.element, sel.start.offset, sel.end.offset);
                        }
                        else {
                            var start = 1;
                            if (sel.start.offset == 1) {
                                start = 0;
                            }
                            else if (sel.start.offset > 1) {
                                start = sel.start.offset - 1;
                            }

                            var end = start + 1;
                            taSelection.setSelection(el, el, start, end);
                        }
                    }
        ...
        taRegisterTool('insertVideo', {
            ...
            action: function () {
                ...
                this.title = taTranslations.insertVideo.dialogPrompt;
                this.urlPrompt = 'https://';
                ...
                template += "  </div>";
                template += "<div class='modal-body'>";
                template += "<legend><i class='fa fa fa-youtube-play'></i> {{title}} </legend>";

                template += "<input tabindex='-1' class='form-control'  ng-model='urlPrompt'  type='text'>";

                template += "</div><div class='modal-footer'>";
                template += "<button  ng-click='Ok(urlPrompt)' class='btn btn-primary btn-login-popap'>Ok</button></div>";
        ...
        taRegisterTool('insertLink', {
            ...
            action: function () {
                ...
                this.title = taTranslations.insertLink.dialogPrompt;
                this.urlLink = null;
                if (taSelection.getSelectionElement().tagName && taSelection.getSelectionElement().tagName.toLowerCase() === 'a') {
                    this.urlLink = taSelection.getSelectionElement().href;
                } else {
                    this.urlLink = 'http://';
                }
                ...
                this.Ok = function (urlLink) {
                    ...
                    if (urlLink && urlLink !== '' && urlLink !== 'http://') {
                        if (!blockJavascript(urlLink)) {
                            this.$editor().wrapSelection('createLink', urlLink, true);
                        }
                    }
